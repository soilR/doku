...
options(tinytex.verbose = TRUE) # https://yihui.org/tinytex/r/#debugging
...
```{r eval = F, echo = F}
# Prepare the data
# This is only done once and this code chunk dont needs to be further evaluated
# Daten Speicherkoog einlesen
readr::read_delim("data/data.csv", delim = ";",
                          col_types = readr::cols(
                            # date = überschreibt die Spalte mit der neuen Spezifikation
                            date = readr::col_datetime(format = "%d.%m.%Y %H:%M") 
  )) %>%
  janitor::clean_names() %>%
  filter(date < as.POSIXct("2010-05-01 00:00")) %>%
  write.csv("data/data_koog_example.csv")

koog_raw <- readr::read_delim("data/data.csv", lazy = F, delim = ";", 
                          col_types = readr::cols(
                            # date = überschreibt die Spalte mit der neuen Spezifikation
                            date = readr::col_datetime(format = "%d.%m.%Y %H:%M") 
  )) %>% janitor::clean_names() %>%
...
# write hourly data
readr::write_csv(x = koog_raw, file = "data/data_hourly.csv")
# Tagesmittelwerte
koog_day <- koog_raw %>%
  group_by(daily) %>%
  ...
  select(!contains("oxy"))
# write daily data
readr::write_csv(x = koog_day, file = "data/data_daily.csv")
# Monatsmittelwerte
koog_month <- koog_raw %>%
  group_by(monthly) %>%
  summarise_if(is.numeric, mean, na.rm = T)
...
koog_year <- koog_raw %>%
  ...
  summarise_if(is.numeric, mean, na.rm = T)
# write yearly data
readr::write_csv(x = koog_year, file = "data/data_year.csv")
